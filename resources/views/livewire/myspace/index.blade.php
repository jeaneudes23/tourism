...
<div class="grid gap-6 lg:grid-cols-[300px,1fr] items-start container">
    <div class="grid py-8 content-start lg:sticky lg:top-16">
        @if (auth()->user()->role == 'manager')
        <button wire:click="tabber('facilities')" class="px-4 justify-start flex gap-1 py-2 rounded-md font-medium text-sm tracking-wide transition-all {{$tab == 'facilities' ? 'bg-primary/10 text-primary' : 'hover:bg-gray-100' }}">Facilities</button>
        @endif
